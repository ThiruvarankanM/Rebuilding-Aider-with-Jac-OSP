...
def get_parser(default_config_files, git_root, formatter_class=None):
    """Create and configure the argument parser"""
    parser_kwargs = {
        "description": "Aider is AI pair programming in your terminal - Version 2.0",
    ...
    # Add formatter class if provided
    if formatter_class:
    ...
    # NEW: Genius mode argument for version 2.0
    group.add_argument(
        "--genius-mode",
        action="store_true",
        help="Enable genius mode for enhanced AI capabilities (Version 2.0 feature)",
        default=False,
    )
    group.add_argument(
        "--weak-model",
    ...
    group.add_argument(
        "--test-cmd",
        action="append",  # Fixed: changed from help with default=[] to action="append"
        help="Specify command to run tests (can be used multiple times)",
        default=[],
    )
    group.add_argument(
        "--auto-test",
    ...
    # Shell completions - using sorted list for consistent order
...
def get_md_help():
    ...
    # Use clean argv for consistent output
    original_argv = sys.argv.copy()
    sys.argv = ["aider"]
    
    try:
        parser = get_parser([], None, MarkdownHelpFormatter)
...
def get_sample_yaml():
    ...
    try:
        parser = get_parser([], None, YamlHelpFormatter)
        parser.parse_known_args()
        return argparse.ArgumentParser.format_help(parser)
    finally:
        sys.argv = original_argv
...
def get_sample_dotenv():
    """Generate sample dotenv configuration"""
    os.environ["COLUMNS"] = "120"
    original_argv = sys.argv.copy()
    sys.argv = ["aider"]
    
    try:
        parser = get_parser([], None, DotEnvFormatter)
        parser.parse_known_args()
        return argparse.ArgumentParser.format_help(parser)
    finally:
        sys.argv = original_argv
...
def main():
    """Main entry point for the args utility"""
    if len(sys.argv) > 1:
        command = sys.argv[1]
    else:
    ...
    try:
        if command == "md":
            ...
            # NEW: Added genius mode command
            print("🧠 Aider Genius Mode Configuration")
            print("=" * 40)
            print("Genius mode enhances AI capabilities with:")
            print("• Advanced reasoning and planning")
            print("• Multi-step problem solving")
            print("• Enhanced code analysis")
            print("• Improved architectural suggestions")
            print("\nTo enable genius mode, use: --genius-mode")
            print("This feature is available in Aider Version 2.0+")
        elif command == "completion":
            if len(sys.argv) > 2:
                shell = sys.argv[2]
                supported_shells = sorted(list(shtab.SUPPORTED_SHELLS))
                if shell not in supported_shells:
                    print(f"Error: Unsupported shell '{shell}'.", file=sys.stderr)
                    print(f"Supported shells are: {', '.join(supported_shells)}", file=sys.stderr)
                    return 1
            ...
            # Default to YAML for any other unrecognized argument
            print(get_sample_yaml())
